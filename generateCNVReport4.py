...
for sample in samples_list 
    ...
    QC_table= pandas.read_table("/data/results/" +runid+ "/IlluminaTruSightCancer/combined_QC.txt")
    num_rows_QC_table= QC_table.shape[0]
    i=0
    while (i<num_rows_QC_table):
        if ((QC_table.iloc[i,0])==sample):
            QC=QC_table.iloc[i,1]
        i=i+1
    ...
    if (Manta_Exome is not None):
        ...
        while (a<num_rows_Manta_Exome):
            ...
            while (b<num_rows_bed_file):
                ...
                if ( (Manta_Exome.iloc[a,0]== bed_file.iloc[b,0]) and(Manta_Exome.iloc[a,2]>= bed_file.iloc[b,2]) and (Manta_Exome.iloc[a,1]<= bed_file.iloc[b,1])) :
