...
samples_list=[]
Exome_depth_metrics= pandas.read_table(runid +"/IlluminaTruSightCancer/" + runid+ "_ExomeDepth_Metrics.txt") 
...
for sample in samples_list: 
    bed_file= pandas.read_table(runid+ "/IlluminaTruSightCancer/tester.bed")
    depth_of_coverage_summary=pandas.read_table(runid+ "/IlluminaTruSightCancer/" +sample+ "/"+ runid+ "_"+ sample+ "_DepthOfCoverage.sample_summary")
    depth_of_coverage_summary_mean=depth_of_coverage_summary['mean'][1]
    
    #Create dataframe from manta vcf
    manta_dataframe=allel.vcf_to_dataframe(runid+ "/IlluminaTruSightCancer/" +sample +"/" +runid + "_" + sample + "_sv_filtered.vcf" , fields=['*'])
    ...
    Exome_dataFrame=allel.vcf_to_dataframe(runid+ "/IlluminaTruSightCancer/" +sample+ "/" +runid+ "_" + sample + "_cnv.vcf", fields=['*'])
